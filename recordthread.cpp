...
RecordThread::RecordThread(QString videoPath, QMutex *lock): running(false), cameraID(-1), videoPath(videoPath), data_lock(lock){
    fps_calculating = false;
    fps = 0.0;
...
void RecordThread::calculateFPS(cv::VideoCapture &cap){
...
void RecordThread::startSavingVideo(cv::Mat &firstFrame){
...
void RecordThread::stopSavingVideo(){
    video_saving_status = STOPPED;
...
void RecordThread::motionDetect(cv::Mat &frame){
    cv::Mat fgmask;
